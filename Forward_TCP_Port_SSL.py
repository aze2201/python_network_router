	...
	while True:
		try:
			...
			clientSocketSSL = ssl.wrap_socket(clientSocket,
                                 server_side=True,
                                 certfile="/etc/nginx/ssl/3bash.com.crt",
                                 keyfile="/etc/nginx/ssl/3bash.com.key")
			## END add by Fariz
		except KeyboardInterrupt:
			print "\nTerminating..."
			terminateAll = True
			break
		ClientThread(clientSocketSSL, targetHost, targetPort).start()
